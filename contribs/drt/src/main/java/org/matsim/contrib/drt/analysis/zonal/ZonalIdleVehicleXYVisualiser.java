...
public class ZonalIdleVehicleXYVisualiser
	...
	public void notifyIterationEnds(IterationEndsEvent event) {
		...
		// how to get the length [sec] of the simulation?
		double simulationLength = 60 * 60 * 24;
		...
		if( numberOfIteration == lastIteration ) {
			...
			this.zoneEntries.forEach(( zone, entriesList ) -> {
				...
				entriesList.forEach(entry -> {
					...
					// bullshit print proof
					System.out.println("ZoneId: " + zoneId +
									", Time: " + time +
									", nVehic: " + numberOfIdleVehicles
					);
				});
			...
			for( int i = 0; i < testDataSet.getSeriesCount(); i++) {
				...
				for(Object o : xySeries.getItems())   {
					...
					double time = (double) item.getXValue();
					double numberOfIdleVehicles = (double) item.getYValue();

					// System.out.println("zoneId " + zoneId + ": " + time + ", " + numberOfIdleVehicles);
			...
			// bullshit print proof
/*			for( int i = 0; i < dataSetPlot.getSeriesCount(); i++) {
				String name = dataSetPlot.getSeriesKey(i).toString();
				System.out.println("Begin Serie " + name);
				XYSeries xySeries = dataSetPlot.getSeries(i);
